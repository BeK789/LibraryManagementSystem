...
<table class="display table table-bordered " id="myTable">
	...
	<?php 
			...
			$sel=" select books.Book_Name, books.ID, students.Name, issue_books.Issue_date, issue_books.status,
			issue_books.id from issue_books
		...
		while($row=$res->fetch_assoc()){ if($row['status']==1) { ?>
